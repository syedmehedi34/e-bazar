...
const ProductsList = () => {
  ...
  const getProductsData = useCallback(async () => {
    const res = await axios.get(
      `https://e-bazaar-server-three.vercel.app/admin/products/list?page=${currentPage}&sort=${sort}&search=${search}`,
      ...
      if (result.isConfirmed) {
        try {
          const res = await axios.delete(
            `https://e-bazaar-server-three.vercel.app/admin/products/${id}`,
          );
          if (res.status === 200) {
      ...
      <div>
        ...
        <div className="bg-white dark:bg-gray-800 dark:text-white p-4">
          <ProductsTable
            ...
            onDelete={handleProductsDeletedById}
          />
          <Pagination
