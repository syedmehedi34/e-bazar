              ...
              <ul className="ml-4">
                {Array.from(categories[category]).map((sub) => (
                  <li
                    ...
                    className="flex w-full justify-between items-center btn btn-sm btn-outline mb-2 border-gray-300 dark:hover:bg-gray-800 dark:text-white dark:border-gray-600"
                    key={sub}
                  >
                    {sub} <MdOutlineArrowRight />
                  </li>
                ))}
              </ul>
            )}
