  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await axios.patch(
        `https://e-bazaar-server-three.vercel.app/admin/user-update/${selectedUser._id}`,
        updatedUser,
      );
      if (res.status === 200) {
    ...
    if (result.isConfirmed) {
      try {
        const res = await axios.delete(
          `https://e-bazaar-server-three.vercel.app/admin/user-delete/${id}`,
        );
        if (res.status === 200) {
  ...
  return (
    <div className="overflow-x-auto w-full p-4 relative">
      ...
      <p className="mb-4 text-gray-600 dark:text-gray-300">
        This table displays the list of all registered users, including their
        names, emails, roles, and registration dates.
      </p>
